...
def build_vector_store(docs, metadatas, ids, reset: bool = False):
    ...
    # Chroma has a max batch size; insert in chunks of 200
    BATCH = 200
    ...
    print(f"[ingest] Indexed {len(docs)} chunks into ChromaDB at {CHROMA_DIR}")
    return collection


# ---------------------------------------------------------------------------
# Step 5: Query interface (used by Agent 2)
# ---------------------------------------------------------------------------
...
def main():
    ...
    parser.add_argument("--repo", help="GitHub repo URL to ingest")
    ...
    if args.repo:
        ...
        print(f"[ingest] Found {len(files)} relevant files")
        docs, metas, ids = chunk_files(files, repo_path)
        build_vector_store(docs, metas, ids, reset=True)
